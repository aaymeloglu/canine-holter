...
- Beat detection: the remaining PVC false positives are motion noise in active hours (20 of 93 in the 08:25-09:25 hour of the 2026-08-25 recording) and residual T waves in sinus-arrhythmia swings; per-beat noise rejection (template-correlation SQI) is the next detection item and depends on the search-back being in place. Lying down is a lead-axis problem - Ch 3 shows a 3.5 mV QRS where Ch 1 shows a 0.03-0.3 mV notch, and the detector lands on the tall P wave of a wandering pacemaker on alternate beats - and multi-lead detection is the structural fix; the `lying_t` fixture is its acceptance test in waiting.
