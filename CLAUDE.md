...
The GUI ships as a signed, notarized macOS `.app` via GitHub Releases so a non-technical user can download and run it with no Python. `canine-holter.spec` (PyInstaller, onedir) builds it; `.github/workflows/release.yml` (triggered on `v*` tags) builds, codesigns, notarizes, and uploads the DMG. Tagging `vX.Y.Z` and pushing the tag cuts a release.
