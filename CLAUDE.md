...
- `arrhythmia/burden.py`: produces `ArrhythmiaSummary`, including burden, runs (consecutive `V`; consecutive `E` are escape couplets and runs), escape-beat times, sinus intervals (an escape beat interrupts a sinus arrest without ending it, so the longest sinus interval can exceed the longest RR), pauses (at 2.5 s and 5 s), rate events and rate shares (at the cardiologist's 60 bpm line and at the weight-class threshold), heart-rate statistics, heart-rate variability (SDNN, RMSSD, pNN50 over NN intervals), quality accounting, and hourly rows, which align to clock hours when the recording start is known.
- `report/`: builds plain `ReportContent`, then renders `report.pdf`. Reports include the disclaimer, six summary panels with reference bands, quality accounting, timeline, hourly table, and reviewable ECG strips (diary-button presses first, then the flagged beats), ending with one strip per hour. Strip caps must always be stated. The cardiologist's HE/LX report is the external oracle; `docs/superpowers/specs/2026-09-02-cardiologist-report-parity-design.md` records how its numbers map to ours.
...
- Bytes 466..470 of an ECG block are the recording sequence number and recorder serial (u16 each); the parser stops at the first ECG block whose pair differs from the first block's, because a reused card keeps older recordings' blocks with valid checksums and sometimes contiguous source positions. Bytes 470..471 are the diary-button field, non-zero only in the block being written when the button was pressed; they are not part of the key, or the recording would end at the first press.
...
- Diary-button presses are read from DR200 files; the DR400 is assumed to store them the same way but no DR400 press has been recorded.
